...
# Misc utility functions for working with hashes and hash files (sometimes called dictionaries).
...
# simpleDict, doubleDict, tripleDict, numDict, simpleAppendNumDict, tampaPhoneNumDict, dateDict
...
# make simpleDict file with words from dictionary along with respective hashes
# dictionary.txt is a simple txt file with one word per line (no hashes)
def makeSimpleDictFile():
    print('Generating simpleDict...')
    with open('dictionary.txt', 'r', encoding='utf-8-sig') as f, open('simpleDict.txt', 'w') as g:
        for line in f:
            line = line.strip() # strip out newlines. Otherwise, it would be included in hash.
            hash = (compute_SHA1_hash(line)) # calc hash
            g.write(f'{line} {hash}\n')

# make numDict hash file (numeric brute force from 0 to 9,999,999)  
def makeNumDictFile():
    print('Generating numDict file...')
    with open('numDict.txt', 'w') as f:
        percentDone = 0
...
# Uses numDict hash file so if that hash file doesn't exist, this will generate it.
def makeTampaPhoneNumDictFile(attempts=0): # pass num of attempts to prevent recursive loop
    if attempts == 2:
        ...
        pass
    try:
        with open('tampaPhoneNumDict.txt', 'w') as f, open('numDict.txt', 'r') as g:
            percentDone = 0
            for line in g:
        ...
        print('Need numDict to generate tampaPhoneNumDict.')
        makeNumDictFile()
        print('Done generating numDict.')
        makeTampaPhoneNumDictFile(attempts+1) # ooooo, recursive function! Fancy!
        print('Done')
    
# Make doubleDict hash file (every combination of two words from simpleDict)
def makeDoubleDictFile():
    print('Generating doubleDict...')
    with open('doubleDict.txt', 'w') as f:
        i = 0
        percentDone = 0
        g = readHashFile('dictionary.txt') # read dictionary into memory
        print('Continue generating doubleDict...')
...
def makeDateDictFile():
    print('Generating dateDict...', end='')
    with open('dateDict.txt', 'w') as f:
...
# create tripleDict (every combination of three words)
#tripleDict = {}
#for word in doubleDict.keys():
